...
**症狀：** `/add_image` 命令响應很慢或超時

**原因：**
- Gemini API 請求耗時較長（5-15 秒）
- 使用了較慢的模型（gemini-1.5-pro）
- 網路連接不穩定
...
```python
# bot.py 中修改初始化
gemini = GeminiClient(model_name="gemini-1.5-flash")  # 最快
```

❌ **避免：**
```python
gemini = GeminiClient(model_name="gemini-1.5-pro")  # 較慢但更準確
```
