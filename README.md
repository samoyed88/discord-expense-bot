...
Bot 會自動檢測相同日期和**相似描述**的支出，**防止重複新增**：

✅ **智能去重**：
- 支持模糊匹配（「連加*停車大聲公」= 「停車大聲公」）
- 移除常見前綴（「連加*」、「連支*」、「foodpanda-」等）
- 大小寫不敏感（「CITYWASH」= 「citywash」）
- 精確匹配優先，模糊匹配備選

| 情況 | 行為 | 提示 |
|------|------|------|
| 新記錄 | ✅ 新增 | 顯示「記錄成功」 |
| 日期和描述相同 | ⚠️ 跳過 | 顯示「記錄已存在」 |
| 日期不同 | ✅ 新增 | 允許記錄（不同日期） |
| 描述不同 | ✅ 新增 | 允許記錄（描述不同） |
...
```
✅ 第一次：連加*停車大聲公 (2026-03-09)
   保存為「連加*停車大聲公」

⚠️ 第二次：停車大聲公 (2026-03-09)
   規範化為「停車大聲公」
   → 匹配成功，拒絕重複
```
...
|------|------|---------|
| 連加* | 連加*停車大聲公 | 停車大聲公 |
| 連支* | 連支*CityWash | citywash |
| foodpanda- | foodpanda-pizza | pizza |
| 和雲行動服務 | 和雲行動服務 iRent | irent |
| MOMO-EC | MOMO-EC 購物 | ec購物 |
| 樂購蝦皮 | 樂購蝦皮-furniture | furniture |

### 重複記錄示例

**第一次記錄：**
```
/add amount:100 category:食物 description:午餐 date:2026-03-15
✅ 記錄成功 - ID: 42
```
...
/add amount:100 category:食物 description:午餐 date:2026-03-15
⚠️ 記錄已存在
相同日期和內容的記錄已存在，不會重複新增
現有記錄 ID: 42
```

**第三次嘗試（有商戶前綴但相同內容）：**
```
/add amount:100 category:食物 description:連加*午餐 date:2026-03-15
⚠️ 記錄已存在
相同日期和內容的記錄已存在，不會重複新增
現有記錄 ID: 42
```
